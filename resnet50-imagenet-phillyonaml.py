...
def run(data_loader_name, datapath, batch_size):
    num_workers = 4
    data_path = datapath
...
def init_processes(master, port, data_loader_name, datapath, batch_size, global_rank, size, fn, backend='nccl'):
    os.environ['MASTER_ADDR'] = master
    os.environ['MASTER_PORT'] = port
    # print("world: ", size, " rank: ", global_rank)
    dist.init_process_group(backend, rank=global_rank, world_size=size)
    fn(data_loader_name, datapath, batch_size)
...
if __name__ == "__main__":
    ...
    for local_rank in range(node_size):
        p = Process(target=init_processes, args=(
            args.master, args.port, args.dataset, args.datapath, batch_size, (node_id * node_size) + local_rank, node_size * world_size, run))
        p.start()
        processes.append(p)
